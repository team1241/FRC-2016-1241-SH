...
public class AutoCourtyard extends CommandGroup {
	...
	public void original(int location) {
		if(location==1) {
			//Turn towards tower
			addParallel(new DriveCommand(137.5, 1, 0, 3));
			addSequential(new TimedTurnTurret(-70, 1, 3, false));
		}
		else if(location==2) {
			//Turn towards tower
			//addSequential(new DrivePath("0,0","22.6,40","36,0","36,122",3.5,0.5));
			addSequential(new DriveCommand(105,1,8,3));
		}
		else if(location==3) {
			addParallel(new DriveCommand(95, 1,0,2));
	...
	public void center(int location) {
		if (location == 1) {
			// addSequential(new
			// DrivePath("0,0","50,75","93,0","93,118",3.5,0.5));
			addParallel(new DriveCommand(115, 1, 65, 3));
			addSequential(new TimedTurnTurret(50, 1, 3, false));
		} else if (location == 2) {
			original(location);
		} else if (location == 3) {
			original(location);
